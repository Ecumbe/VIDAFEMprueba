...
function postAgendaApiJson_(payload) {
    ...
    return fetch(buildAgendaApiUrl_(), {
        method: "POST",
        cache: "no-store",
...
function loadAgenda(dateString) {
    ...
    postAgendaApiJson_({ action: "get_agenda", fecha: dateString, requester: requester })
    .then(res => {
        container.innerHTML = "";
...
function setApptStatus(id, status, btn) {
    ...
    postAgendaApiJson_({ action: "update_appt_status", id_cita: id, estado: status, requester: requester });
}
...
if (formResch) {
    formResch.addEventListener("submit", function(e) {
        ...
        postAgendaApiJson_({ action: "reschedule_appointment", data: data, requester: requester })
        .then(res => {
            if (res.success) {
